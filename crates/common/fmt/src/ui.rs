...
use alloy_consensus::{
    ...
    TxReceipt, TxType, Typed2718, transaction::TxHashRef,
};
use alloy_network::{
...
impl UIfmt for TempoTxEnvelope {
    fn pretty(&self) -> String {
        match self {
            Self::Legacy(tx) => TxEnvelope::Legacy(tx.clone()).pretty(),
            Self::Eip2930(tx) => TxEnvelope::Eip2930(tx.clone()).pretty(),
            Self::Eip1559(tx) => TxEnvelope::Eip1559(tx.clone()).pretty(),
            Self::Eip7702(tx) => TxEnvelope::Eip7702(tx.clone()).pretty(),
            Self::AA(_) => format!(
                "
gas                  {}
gasPrice             {}
...
impl UIfmtReceiptExt for TempoTransactionReceipt {
    fn logs_pretty(&self) -> String {
        serde_json::to_string(&self.inner.inner.receipt.logs).unwrap_or_default()
    }
