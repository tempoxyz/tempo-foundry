...
fn convert_executed_result(
    ...
    let (exit_reason, gas_refunded, gas_used, out, exec_logs) = match result {
        ...
        ExecutionResult::Revert { gas, output, .. } => {
            (InstructionResult::Revert, 0_u64, gas.tx_gas_used(), Some(Output::Call(output)), vec![])
        }
        ExecutionResult::Halt { reason, gas, .. } => {
            ...
            (instruction_result, 0_u64, gas.tx_gas_used(), None, vec![])
        }
    };
    let init_gas = revm::interpreter::gas::calculate_initial_tx_gas(
    ...
    Ok(RawCallResult {
        ...
        gas_used,
        gas_refunded,
        stipend: init_gas.initial_total_gas,
        logs,
        labels,
        traces,
        line_coverage,
