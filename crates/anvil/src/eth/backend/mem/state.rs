...
use revm::{database::DbAccount, primitives::StorageKeyMap, state::AccountInfo};
...
pub fn storage_root(storage: &StorageKeyMap<U256>) -> B256 {
    build_root(trie_storage(storage))
}

/// Builds iterator over stored key-value pairs ready for storage trie root calculation.
pub fn trie_storage(storage: &StorageKeyMap<U256>) -> Vec<(Nibbles, Vec<u8>)> {
...
pub fn trie_account_rlp(info: &AccountInfo, storage: &StorageKeyMap<U256>) -> Vec<u8> {
