...
pub struct TempoAccessKeyConfig {
    ...
    /// Default fee token derived from the key's spending limits.
...
pub fn lookup_signer(from: Address) -> Result<TempoLookup> {
    ...
    for entry in &file.keys {
        ...
        let fee_token = entry.limits.first().map(|l| l.currency);
...
pub async fn sign_with_access_key(
    ...
    let aa_signed = AASigned::new_unhashed(tempo_tx, TempoSignature::Keychain(keychain_sig));

    let envelope = TempoTxEnvelope::from(aa_signed);
