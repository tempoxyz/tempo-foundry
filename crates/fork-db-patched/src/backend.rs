...
impl DatabaseRef for SharedBackend {
    ...
    fn basic_ref(&self, address: Address) -> Result<Option<AccountInfo>, Self::Error> {
        // Short-circuit Tempo system precompiles.
        // These are Rust-native precompiles whose state lives in the Tempo EVM,
        // not in on-chain storage. Querying the RPC returns null data, and during
        // invariant fuzzing the repeated ~200ms round-trips effectively hang the runner.
        let addr_bytes = address.as_slice();
        if matches!(
            (addr_bytes[0], addr_bytes[1]),
            (0xDE, 0xC0) | (0xFE, 0xEC) | (0x40, 0x3C) | (0xA0, 0x00) | (0xF0, 0x00) | (0x20, 0xC0)
        ) {
            let dummy_code = Bytecode::new_raw(vec![0xFE].into()); // INVALID opcode
            return Ok(Some(AccountInfo {
                balance: U256::ZERO,
                nonce: 1,
                code_hash: keccak256(vec![0xFE]),
                account_id: None,
                code: Some(dummy_code),
            }));
        }
